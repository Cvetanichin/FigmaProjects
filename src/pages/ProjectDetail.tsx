...
import { ArrowLeft, Pencil, Target, AlertTriangle, Activity, FileText, Bot, FileOutput, CalendarDays, DollarSign } from 'lucide-react'
...
export function ProjectDetail() {
  ...
  const currentTab = tabs.find(t =>
    t.path === '' ? location.pathname === `/projects/${id}` : location.pathname.endsWith(t.path)
  )
    ...
    <div className="flex flex-col h-full">
      <div className="border-b border-border px-6 pt-5 pb-0">
        ...
        <div className="flex items-start justify-between mb-4">
          ...
          <button onClick={() => setEditing(true)} className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors px-2 py-1.5 rounded-md hover:bg-accent">
        ...
        <nav className="flex gap-0.5 -mb-px overflow-x-auto">
          {tabs.map(({ path, label, icon: Icon }) => {
            ...
            const isActive = path === '' ? location.pathname === to : location.pathname === to
            return (
              <Link
                key={path}
                to={to}
